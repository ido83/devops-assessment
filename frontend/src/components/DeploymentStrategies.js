...
import { DEPLOY_TEMPLATES, DEPLOY_TEMPLATE_LIST, DEPLOY_NODE_COLORS, DEPLOY_NODE_ICONS } from '../data/deploymentTemplates';
const NODE_TYPES = Object.keys(DEPLOY_NODE_COLORS);

function StrategyDiagram({ flow, onChange, onDelete, toast }) {
  const [editId, setEditId] = useState(null);
  const [addModal, setAddModal] = useState(false);
  const [addForm, setAddForm] = useState({ label:'',type:'deploy',sub:'',connectFrom:'',connectTo:'' });
  ...
  return (
    <div className="pipeline-card">
      <div className="pipeline-header">
        <div style={{flex:1}}>
          <input className="pipeline-title-input" value={flow.name} onChange={e=>onChange({...flow,name:e.target.value})} placeholder="Strategy name"/>
        ...
        <div style={{display:'flex',gap:4,flexWrap:'wrap'}}>
          ...
          <div className="pipeline-export-group">
            <button className="btn btn-ghost btn-xs" onClick={()=>{exportAsJSON({name:flow.name,description:flow.description,cat:flow.cat,nodes,edges},flow.name||'strategy');toast?.('JSON exported','success');}}>JSON</button>
            <button className="btn btn-ghost btn-xs" onClick={()=>{exportSVGElement(svgRef.current,flow.name||'strategy',svgW,svgH);toast?.('SVG exported','success');}}>SVG</button>
            <button className="btn btn-ghost btn-xs" onClick={()=>{exportSVGAsPNG(svgRef.current,flow.name||'strategy',svgW,svgH);toast?.('PNG exported','success');}}>PNG</button>
      ...
      <div className="pipeline-svg-wrap">
        <svg ref={svgRef} width={svgW} height={svgH} viewBox={`0 0 ${svgW} ${svgH}`} className="pipeline-svg" data-section="deploy" data-diagram-name={flow.name || 'strategy'}>
          <defs><marker id="dsah" markerWidth="8" markerHeight="6" refX="8" refY="3" orient="auto"><polygon points="0 0,8 3,0 6" fill="var(--text-muted)"/></marker></defs>
          ...
          {nodes.map(n=>{const p=positions[n.id];if(!p)return null;const c=DEPLOY_NODE_COLORS[n.type]||'#636e72';const sel=editId===n.id||connectMode?.from===n.id;return(<g key={n.id} style={{cursor:connectMode?'crosshair':'pointer'}} onClick={()=>handleClick(n.id)}>
            ...
            <text x={p.x} y={p.y+1} textAnchor="middle" fontSize="18" dominantBaseline="central">{DEPLOY_NODE_ICONS[n.type]||'⚙️'}</text>
      ...
      <div className="pipeline-stage-list">
        ...
        {nodes.map(n=>{const oc=edges.filter(e=>e.from===n.id).length,ic=edges.filter(e=>e.to===n.id).length;return<div key={n.id} className={`pipeline-stage-row ${editId===n.id?'active':''}`} onClick={()=>setEditId(editId===n.id?null:n.id)}><span className="pipeline-stage-dot" style={{background:DEPLOY_NODE_COLORS[n.type]||'#636e72'}}/><span className="pipeline-stage-label">{n.label}</span>{ic>0&&<span className="pipeline-conn-badge in">←{ic}</span>}{oc>0&&<span className="pipeline-conn-badge out">→{oc}</span>}<span className="pipeline-stage-type">{n.type}</span></div>;})}
      </div>
      {editId&&(()=>{const n=nodes.find(x=>x.id===editId);if(!n)return null;const outs=edges.filter(e=>e.from===n.id);const ins=edges.filter(e=>e.to===n.id);return<div className="pipeline-editor"><div className="config-grid" style={{gap:10}}><div className="config-field"><label>Label</label><input type="text" value={n.label} onChange={e=>upN(n.id,'label',e.target.value)}/></div><div className="config-field"><label>Type</label><select value={n.type} onChange={e=>upN(n.id,'type',e.target.value)}>{NODE_TYPES.map(t=><option key={t} value={t}>{DEPLOY_NODE_ICONS[t]} {t}</option>)}</select></div></div><div className="config-field" style={{marginTop:8}}><label>Description</label><input type="text" value={n.sub||''} onChange={e=>upN(n.id,'sub',e.target.value)}/></div><div style={{marginTop:8,display:'flex',gap:6,flexWrap:'wrap'}}><label style={{fontSize:10,color:'var(--text-muted)',width:'100%',fontFamily:"'JetBrains Mono',monospace"}}>CONNECTIONS</label>{ins.map(e=><div key={e.from+e.to} className="conn-chip"><span>←{nodes.find(x=>x.id===e.from)?.label||'?'}</span><button className="conn-remove" onClick={()=>rmE(e.from,e.to)}>✕</button></div>)}{outs.map(e=><div key={e.from+e.to} className="conn-chip"><span>→{nodes.find(x=>x.id===e.to)?.label||'?'}</span><button className="conn-remove" onClick={()=>rmE(e.from,e.to)}>✕</button></div>)}<select className="conn-add-select" defaultValue="" onChange={e=>{if(e.target.value){addE(n.id,e.target.value);e.target.value='';}}}><option value="" disabled>+ connect →</option>{nodes.filter(x=>x.id!==n.id&&!outs.some(o=>o.to===x.id)).map(x=><option key={x.id} value={x.id}>{DEPLOY_NODE_ICONS[x.type]} {x.label}</option>)}</select></div><div style={{display:'flex',gap:8,marginTop:10}}><button className="btn btn-ghost btn-xs" onClick={()=>setEditId(null)}>Done</button><button className="btn btn-ghost btn-xs" style={{color:'var(--severity-critical)'}} onClick={()=>rmN(n.id)}>Remove</button></div></div>;})()}
      {addModal&&<div className="modal-overlay" onClick={()=>setAddModal(false)}><div className="modal" onClick={e=>e.stopPropagation()}><div className="modal-header"><h3>Add Stage</h3><button className="btn-icon" onClick={()=>setAddModal(false)}>✕</button></div><div className="modal-body"><div className="config-field"><label>Label *</label><input type="text" value={addForm.label} onChange={e=>setAddForm(p=>({...p,label:e.target.value}))}/></div><div className="config-field"><label>Type</label><div className="stage-type-grid">{NODE_TYPES.map(t=><button key={t} type="button" className={`stage-type-chip ${addForm.type===t?'selected':''}`} style={{'--chip-color':DEPLOY_NODE_COLORS[t]}} onClick={()=>setAddForm(p=>({...p,type:t}))}><span>{DEPLOY_NODE_ICONS[t]}</span><span>{t}</span></button>)}</div></div><div className="config-field"><label>Description</label><input type="text" value={addForm.sub} onChange={e=>setAddForm(p=>({...p,sub:e.target.value}))}/></div><div className="config-grid" style={{gap:8,marginTop:8}}><div className="config-field"><label>From</label><select value={addForm.connectFrom} onChange={e=>setAddForm(p=>({...p,connectFrom:e.target.value}))}><option value="">None</option>{nodes.map(n=><option key={n.id} value={n.id}>{n.label}</option>)}</select></div><div className="config-field"><label>To</label><select value={addForm.connectTo} onChange={e=>setAddForm(p=>({...p,connectTo:e.target.value}))}><option value="">None</option>{nodes.map(n=><option key={n.id} value={n.id}>{n.label}</option>)}</select></div></div></div><div className="modal-footer"><button className="btn btn-secondary" onClick={()=>setAddModal(false)}>Cancel</button><button className="btn btn-primary" onClick={addN} disabled={!addForm.label.trim()}>Add</button></div></div></div>}
    </div>
  );
}

const DeploymentStrategies = ({ data, setData, toast }) => {
  const d = data||{strategies:[]};const strategies=d.strategies||[];
  const [showPicker, setShowPicker] = useState(false);const [filterCat, setFilterCat] = useState('all');
  const persist = useCallback(s=>setData({...d,strategies:s}),[d,setData]);
  const addTpl=(key)=>{const fn=DEPLOY_TEMPLATES[key];if(!fn)return;const t=fn();persist([...strategies,{id:mkId(),...t}]);setShowPicker(false);toast?.(`"${t.name}" added`,'success');};
  const addBlank=()=>{const ids={};const mk=(k)=>{ids[k]=mkId();return ids[k];};persist([...strategies,{id:mkId(),name:'Custom Strategy',description:'',cat:'Custom',nodes:[{id:mk('s1'),label:'Source',type:'source',sub:''},{id:mk('d1'),label:'Deploy',type:'deploy',sub:''},{id:mk('m1'),label:'Monitor',type:'monitor',sub:''}],edges:[{from:ids.s1,to:ids.d1},{from:ids.d1,to:ids.m1}]}]);setShowPicker(false);};
  const cats=['all','Standard','Advanced','Hybrid','Air Gap'];
  const filtered=filterCat==='all'?DEPLOY_TEMPLATE_LIST:DEPLOY_TEMPLATE_LIST.filter(t=>t.cat===filterCat);
  return (
    <div className="animate-in">
      <div className="section-header"><div style={{display:'flex',justifyContent:'space-between',alignItems:'center'}}><div><h2>Deployment Strategies</h2><p>Model and compare deployment patterns — from standard CD to air-gapped environments.</p></div><button className="btn btn-primary" onClick={()=>setShowPicker(true)}>+ Strategy</button></div></div>
      <div className="pipeline-legend">{Object.entries(DEPLOY_NODE_COLORS).map(([k,v])=><div key={k} className="pipeline-legend-item"><span className="pipeline-legend-dot" style={{background:v}}/><span>{DEPLOY_NODE_ICONS[k]}</span><span>{k}</span></div>)}</div>
      {strategies.length===0&&<div className="empty-state" style={{marginTop:40}}><div style={{fontSize:48,marginBottom:16}}>🚀</div><h3>No Strategies</h3><p>Add a deployment strategy to visualize and customize.</p><button className="btn btn-primary" style={{marginTop:16}} onClick={()=>setShowPicker(true)}>+ Strategy</button></div>}
      {strategies.map((s,i)=><StrategyDiagram key={s.id} flow={s} onChange={up=>{const n=[...strategies];n[i]=up;persist(n);}} onDelete={()=>{persist(strategies.filter((_,j)=>j!==i));toast?.('Deleted','success');}} toast={toast}/>)}
      {showPicker&&<div className="modal-overlay" onClick={()=>setShowPicker(false)}><div className="modal modal-lg" onClick={e=>e.stopPropagation()}><div className="modal-header"><h3>Add Deployment Strategy</h3><button className="btn-icon" onClick={()=>setShowPicker(false)}>✕</button></div><div className="modal-body"><div className="filter-bar" style={{marginBottom:12}}>{cats.map(c=><button key={c} className={`filter-chip ${filterCat===c?'active':''}`} onClick={()=>setFilterCat(c)}>{c}</button>)}</div><div className="template-picker-grid">{filtered.map(({key,icon,cat})=>{const t=DEPLOY_TEMPLATES[key]();return<button key={key} className="template-picker-card" onClick={()=>addTpl(key)}><div className="tpl-pick-icon">{icon}</div><div className="tpl-pick-info"><h4>{t.name}</h4><p>{t.description}</p><div style={{display:'flex',gap:4,marginTop:4}}><span className="tpl-pick-count">{t.nodes.length} stages</span><span className="pipeline-tag">{cat}</span></div></div></button>;})}<button className="template-picker-card blank" onClick={addBlank}><div className="tpl-pick-icon">➕</div><div className="tpl-pick-info"><h4>Custom Strategy</h4><p>Blank source → deploy → monitor</p></div></button></div></div></div></div>}
    </div>);};
